...
const BLANK = {
  ...
  seller: "",
  abordado: "", respondeu: "", ofertas: "", vendas: "", valor: "",
  produtos: [], pagamento: "À vista", entrada: "",
  obs: "", insight: "",
};
...
function FormPage({ onAdd }) {
  ...
  const handleSubmit = async () => {
    ...
    const record = { ...f, abordado: num(f.abordado), respondeu: num(f.respondeu), ofertas: num(f.ofertas), vendas: num(f.vendas), valor: num(f.valor), entrada: num(f.entrada) };
...
function ListPage({ data, onDelete, onEdit }) {
  ...
  const filtered = data.filter(d =>
    d.seller?.toLowerCase().includes(search.toLowerCase()) || d.date?.includes(search)
  );
  ...
  const saveEdit = async () => {
    ...
    setSaving(false);
    if (error) { alert("Erro ao salvar: " + error.message); return; }
    onEdit(editData); setEditId(null); setEditData(null);
  };

  const handleDelete = async id => {
    const { error } = await supabase.from("registros").delete().eq("id", id);
    if (error) { alert("Erro ao apagar: " + error.message); return; }
    onDelete(id); setConfirmId(null);
  };
  ...
  return (
    <div>
      ...
      <div style={{ ...css.card, marginBottom: 20, padding: "14px 18px" }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por vendedora ou data..." style={{ ...inputStyle, fontSize: 14 }} />
      </div>
              ...
              {isEditing ? (
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 14, marginBottom: 16 }}>
                    {[
                      ...
                      <div key={k}>
                        ...
                        {type === "select" ? (
                          <select value={ed[k]} onChange={e => setEditData({ ...ed, [k]: e.target.value })} style={{ ...inputStyle, cursor: "pointer" }}>{opts.map(o => <option key={o}>{o}</option>)}</select>
                        ) : (
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <button onClick={saveEdit} disabled={saving} style={{ padding: "9px 24px", borderRadius: 9, border: "none", background: T.green, color: T.white, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 13 }}>{saving ? "Salvando..." : "Salvar"}</button>
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                    <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
                      <div>
                        <div style={{ ...css.label, fontSize: 10 }}>Data</div>
                        <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, color: T.text, fontSize: 14 }}>{r.date}</div>
                      </div>
                      <div>
                        <div style={{ ...css.label, fontSize: 10 }}>Vendedora</div>
                        <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, color: T.green, fontSize: 14 }}>{r.seller}</div>
                      </div>
                      <div>
                        <div style={{ ...css.label, fontSize: 10 }}>Pagamento</div>
                  ...
                  <div style={{ marginTop: 14, display: "flex", gap: 20, flexWrap: "wrap", paddingTop: 12, borderTop: `1px solid ${T.beigeDeep}` }}>
                    {[["Abordados",r.abordado],["Responderam",r.respondeu],["Ofertas",r.ofertas],["Vendas",r.vendas]].map(([l,v]) => (
                      <div key={l}>
    ...
    <div>
        ...
        {[
          ...
          <div key={label} style={{ marginBottom: 10 }}>
            ...
            <div style={{ height: 8, background: T.beigeDeep, borderRadius: 4, overflow: "hidden" }}>
              <div style={{ width: `${p}%`, height: "100%", background: i === 3 ? T.gold : T.green, borderRadius: 4 }} />
            </div>
          </div>
        ))}
      </div>
      ...
      {byProduct.length > 0 && (
        <div style={{ ...css.card, marginTop: 24 }}>
          ...
          {byProduct.map((p, i) => {
            const total2 = byProduct.reduce((s, x) => s + x.qty, 0);
            return (
              <div key={p.nome} style={{ marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    <span style={{ fontSize: 11, color: T.gray }}>{((p.qty / total2) * 100).toFixed(0)}%</span>
                    <span style={{ fontSize: 15, fontWeight: 700, color: PALETTE[i % PALETTE.length], fontFamily: "'Cormorant Garamond', serif" }}>{p.qty} un</span>
                  </div>
                </div>
...
function FilterBar({ data, filters, setFilters }) {
  ...
  return (
    <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:28, padding:"16px 20px", background:T.white, borderRadius:14, border:`1px solid ${T.grayLight}` }}>
      ...
      {(filters.seller||filters.produto||filters.from||filters.to) && (
        <button onClick={() => setFilters({seller:"",produto:"",from:"",to:""})} style={{ ...sel, background:T.beige, color:T.gray }}>✕ Limpar</button>
      )}
    </div>
  );
}
...
export default function App() {
  ...
  const [filters, setFilters] = useState({ seller:"", produto:"", from:"", to:"" });
    ...
    <div style={{ minHeight:"100vh", background:T.beige }}>
      ...
      <div style={{ marginLeft:220, padding:"40px 40px 60px", minHeight:"100vh" }}>
        {loading ? (
          ...
          <>
            {tab === "dash" && <FilterBar data={records} filters={filters} setFilters={setFilters} />}
